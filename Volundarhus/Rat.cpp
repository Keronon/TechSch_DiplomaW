...
void Rat::initGUI()
{
	...
	this->hpBar.setPosition(Vector2f(this->sprite.getPosition()));
}
...
void Rat::update(const float & dt, Vector2f& mouse_pos_view)
{
	this->movementComponent->update(dt);

	//Update GUI Remove this!!!
	this->hpBar.setSize(Vector2f
		(60.f * (static_cast<float>(this->attributeComponent->hp) / this->attributeComponent->hpMax), 10.f)
	);
	this->hpBar.setPosition(Vector2f(this->sprite.getPosition()));
...
void Rat::render(RenderTarget & target, Shader* shader, const Vector2f light_position, const bool show_hitbox)
{
	if (shader)
	{
