...
void TileMap::resizeMap()
{
	...
	for (int x = 0; x < this->maxSizeWorldGrid.x; x++)
	{
		...
		{
			this->map[x].resize(this->maxSizeWorldGrid.y, vector<vector<Tile*>>());
	...
	this->createBorder.setSize(Vector2f(this->maxSizeWorldF.x, this->maxSizeWorldF.y));
}
...
TileMap::TileMap(float gridSize, int width, int height, string texture_file)
{
	...
	this->entityBox.setSize(Vector2f(gridSize, gridSize));
	this->entityBox.setFillColor(Color(255, 100, 255, 50));
	this->entityBox.setOutlineColor(Color::Magenta);
	this->entityBox.setOutlineThickness(1.f);

	this->outmapBox.setSize(Vector2f(this->gridSizeF, this->gridSizeF));
	this->outmapBox.setFillColor(Color(150, 0, 0, 50));
	this->outmapBox.setOutlineColor(Color::White);
	this->outmapBox.setOutlineThickness(1.f);

	this->createBorder.setSize(Vector2f(this->maxSizeWorldF.x, this->maxSizeWorldF.y));
	this->createBorder.setFillColor(Color::Transparent);
	this->createBorder.setOutlineColor(Color::White);
	this->createBorder.setOutlineThickness(1.f);
	this->createBorder.setPosition(0, 0);
}
...
TileMap::TileMap(const string file_name)
{
	...
	this->entityBox.setSize(Vector2f(this->gridSizeF, this->gridSizeF));
	this->entityBox.setFillColor(Color(255, 100, 255, 50));
	this->entityBox.setOutlineColor(Color::Magenta);
	this->entityBox.setOutlineThickness(1.f);
...
void TileMap::setMaxSizeGrid(Vector2i max_size_grid)
{
	this->maxSizeWorldGrid = max_size_grid;

	this->maxSizeWorldF.x = static_cast<float>(this->maxSizeWorldGrid.x) * this->gridSizeF;
	this->maxSizeWorldF.y = static_cast<float>(this->maxSizeWorldGrid.y) * this->gridSizeF;
...
void TileMap::addTile(const int x, const int y, const int z, const IntRect & texture_rect, 
	const int enemy_type, const int enemy_amount, const int enemy_tts, const int enemy_md)
{
		...
		z < this->layers && z >= 0)
	{
		this->map[x][y][z].push_back(new EnemySpawnerTile(x, y, this->gridSizeF, this->tileSheet, texture_rect, 
			enemy_type, enemy_amount, enemy_tts, static_cast<float>(enemy_md)));
	}
}
...
void TileMap::render (
	...
	const bool show_outmap,
	const bool show_create_border)
{
	if (show_create_border)
	{
		target.draw(this->createBorder);
	}

	this->layer = 0;

	this->fromX = gridPosition.x - 150/2;
	if (this->fromX < 0)
		this->fromX = 0;
	else if (this->fromX > this->maxSizeWorldGrid.x)
		this->fromX = this->maxSizeWorldGrid.x;

	this->toX = gridPosition.x + 160/2;
	if (this->toX < 0)
		this->toX = 0;
	else if (this->toX > this->maxSizeWorldGrid.x)
		this->toX = this->maxSizeWorldGrid.x;

	this->fromY = gridPosition.y - 80/2;
	if (this->fromY < 0)
		this->fromY = 0;
	else if (this->fromY > this->maxSizeWorldGrid.y)
		this->fromY = this->maxSizeWorldGrid.y;

	this->toY = gridPosition.y + 90/2;
	if (this->toY < 0)
		this->toY = 0;
	else if (this->toY > this->maxSizeWorldGrid.y)
		this->toY = this->maxSizeWorldGrid.y;
	...
	for (int x = this->fromX; x < this->toX; x++)
	{
		for (int y = this->fromY; y < this->toY; y++)
		{
			for (size_t k = 0; k < this->map[x][y][this->layer].size(); k++)
			{
				...
				if (show_enemy_spawners)
				{
					if (this->map[x][y][this->layer][k]->getType() == TileTypes::ENEMYSPAWNER)
					{
						this->entityBox.setPosition(this->map[x][y][this->layer][k]->getPosition());
				...
				if (show_outmap)
				{
					if (this->map[x][y][this->layer][k]->getType() == TileTypes::OUTMAP)
					{
						this->outmapBox.setPosition(this->map[x][y][this->layer][k]->getPosition());
						target.draw(this->outmapBox);
					}
				}
			}
		}
