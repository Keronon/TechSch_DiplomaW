...
void DefaultEditorMode::initGui()
{
	...
	this->textureSelector = new gui::TextureSelector(
		15.f, 15.f, 500.f, 500.f,
		this->stateData->gridSize, this->tileMap->getTileSheet(),
		*this->editorStateData->font, "Tile\nSet"
	);
...
void DefaultEditorMode::updateInput(const float & dt)
{
	...
	if (Mouse::isButtonPressed(Mouse::Left) && this->getKeytime(13.f))
	{
	...
	else if (Mouse::isButtonPressed(Mouse::Right) && this->getKeytime(13.f))
	{
		if (!this->sidebar.getGlobalBounds().contains(Vector2f(*this->editorStateData->mousePosWindow)))
		{
			if (!this->textureSelector->getActive())
	...
	//Toggle collision
	if (Keyboard::isKeyPressed(Keyboard::Key(this->editorStateData->keybinds->at("TOGGLE_COLLISION"))) && this->getKeytime())
	{
		if (this->collision)
			this->collision = false;
		else
	...
	//Set lock on / off
	if (Keyboard::isKeyPressed(Keyboard::Key(this->editorStateData->keybinds->at("TOGGLE_TILE_LOCK"))) && this->getKeytime())
	{
		if (this->tileAddLock)
			this->tileAddLock = false;
		else
	...
	//Create border
	if (Keyboard::isKeyPressed(Keyboard::Key::Num1) && this->getKeytime())
	{
		if (Keyboard::isKeyPressed(Keyboard::LShift))
		{
			if (this->tileMap->getMaxSizeGrid().x > 1)
				this->tileMap->setMaxSizeGrid(Vector2i(this->tileMap->getMaxSizeGrid().x - 1, this->tileMap->getMaxSizeGrid().y));
		}
		else if (this->tileMap->getMaxSizeGrid().x < 300)
			this->tileMap->setMaxSizeGrid(Vector2i(this->tileMap->getMaxSizeGrid().x + 1, this->tileMap->getMaxSizeGrid().y));
	}
	if (Keyboard::isKeyPressed(Keyboard::Key::Num2) && this->getKeytime())
	{
		if (Keyboard::isKeyPressed(Keyboard::LShift))
		{
			if (this->tileMap->getMaxSizeGrid().y > 1)
				this->tileMap->setMaxSizeGrid(Vector2i(this->tileMap->getMaxSizeGrid().x, this->tileMap->getMaxSizeGrid().y - 1));
		}
		else if (this->tileMap->getMaxSizeGrid().y < 300)
			this->tileMap->setMaxSizeGrid(Vector2i(this->tileMap->getMaxSizeGrid().x, this->tileMap->getMaxSizeGrid().y + 1));
	}
}
